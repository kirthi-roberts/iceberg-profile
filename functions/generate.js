// functions/generate.js — Cloudflare Pages Function
// ─────────────────────────────────────────────────────────────
...
export async function onRequestPost(context) {
  ...
  const { messages, model, max_tokens, bypass_code, tier } = payload;

  // Bypass code check
  const validCodes = (env.BYPASS_CODES || '').split(',').map(c => c.trim().toLowerCase());
  const hasBypass = bypass_code && validCodes.includes(bypass_code.trim().toLowerCase());
